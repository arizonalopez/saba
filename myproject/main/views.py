...
from flask import render_template, session, redirect, url_for, Blueprint, abort, flash, request, current_app, make_response
from .form import PostForm, EditProfileForm, EditProfileAdminForm, CommentForm
from .. import db
...
def index():
    ...
    return render_template('nameform.html', form=form, posts=posts, pagination=pagination, show_followed=show_followed)
...
def unfollow(username):
    ...
    if current_user.is_following(user):
        ...
        db.session.commit()
        flash('You have now unfollowed this user.')
